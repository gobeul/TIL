...
클래스 내부에서 정의되는 메서드는 기본적으로 인스턴스 메소드이다.(데코레이션으로 변경 가능)<br>
메서드 선언시, 첫번째 인자로 인스턴스 자기 자신에 해당하는 `self` 가 전달된다.<br>
메소드 호출시 `self` 에 해당하는 인자는 따로 적지 않는다.

```python
class Person:
    def talk(self): ## 첫번째 인자 명이 self 필요는 없지만 개발자들 사이에서 암묵적인 약속같은 것이다.
        return "안녕!"
    def hi(adafgsg): ## 인스턴스 메소드의 첫 인자는 self로 쓰자는 약속
        return "Hi"
...
생성자 메소드와 속성이 겹치게 되면 생성자 메소드에서 정의한 속성으로 할당된다.<br>
